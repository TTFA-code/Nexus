...
export default function LoginPage() {
    ...
    const handleLogin = async () => {
        const redirectTo = `${location.origin}/auth/callback?next=/dashboard/nexus`
    ...
    return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-black text-white p-8">
            <div className="max-w-md w-full bg-zinc-900 p-8 rounded-lg border border-zinc-800 text-center">
                <h1 className="text-3xl font-bold mb-6">Login to Nexus</h1>
                <p className="text-zinc-400 mb-8">
                    Sign in with Discord to view your personal stats and manage your profile.
                </p>

                <button
                    onClick={handleLogin}
                    className="w-full flex items-center justify-center gap-3 bg-[#5865F2] hover:bg-[#4752C4] text-white font-medium py-3 px-4 rounded-md transition-colors"
                >
